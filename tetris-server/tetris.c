...
int check_possible_pos(int x, int y)
{
     ...
     for(i = 0; i < 4; ++i)
          for(j = 0; j < EXP_FACT; ++j)
               ...
               {
                    ++c;
                    //printf("%d\n",frame[x + shapes[current.num][current.pos][i][0]][y + shapes[current.num][current.pos][i][1] * EXP_FACT + j]);
               }
...
void send_message(void)
{
     ...
     {    
          ...
          else if(server_cmd == 3)
          {
               ...
               char send_msg[1000];
               //printf("send\n");
               //printxy(2, FRAMEH_NB + 5, FRAMEW + 3, "send");
               //printxy(0, FRAMEH_NB + 5, FRAMEW + 3, "send");
               //Inplement commends
               memset(send_msg, '\0', 1000);
               send_msg[0] = 'E';
               send(connfd, send_msg, strlen(send_msg), 0);
